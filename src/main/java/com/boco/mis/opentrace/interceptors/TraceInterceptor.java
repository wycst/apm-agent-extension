...
public class TraceInterceptor {
	...
	public static Object intercept(@Origin Method method,
		...
		// 判断是否为 tomcat的请求资源
		if(method.getDeclaringClass().getName().equals("org.apache.catalina.connector.CoyoteAdapter") && method.getName().equals("service")) {
			...
			try {
				...
				if(requestURI.toLowerCase().matches(ignoreRequestPattern)) {
					...
					globalTrace.setHttpMethod(MVEL.eval("request.method()", vars) + "");
					globalTrace.setQueryString(MVEL.eval("request.queryString()", vars) + "");
					...
					String scheme = MVEL.eval("request.scheme()", vars) + ""; 
					if(scheme.equals("null")) {
						scheme = "http";
					}
					String serverName = MVEL.eval("request.serverName()", vars) + "";
					String serverPort = MVEL.eval("request.getServerPort()", vars) + "";
...
//					globalTrace.addServer(webServer);
					...
					String requestURL = scheme + "://" + serverName + ":" + serverPort + requestURI;
		...
		// HttpServlet
		if(method.getDeclaringClass().getName().equals("javax.servlet.http.HttpServlet") && method.getName().equals("service")) {
			Object request = args[0];
			Map<String,Object> vars = new HashMap<String,Object>();
			vars.put("request", request);
			String requestURI = MVEL.eval("request.getRequestURI()", vars) + "";
			if(requestURI.toLowerCase().matches(ignoreRequestPattern)) {
				// 静态资源
			} else {
				// service会被调用2次，一次是  实现GenericServlet的public(参数servletRequest)，一次是内部定义的protected（参数httpservletrequest）
				if(globalTrace == null) {
					isGlobalTrace = true;
					globalTrace = new GlobalTrace();
					globalTraceThreadLocal.set(globalTrace);
				...
				if(!"Servlet".equals(globalTrace.getHttpComponent())) {
					...
					globalTrace.setHttp(true);
					globalTrace.setHttpMethod(MVEL.eval("request.getMethod()", vars) + "");
					globalTrace.setQueryString(MVEL.eval("request.getQueryString()", vars) + "");
					globalTrace.setRequestURI(MVEL.eval("request.getRequestURI()", vars) + "");
					globalTrace.setTraceName(globalTrace.getRequestURI());
					globalTrace.setRequestURL(MVEL.eval("request.getRequestURL()", vars) + "");
				}
			}
		}
		...
		if(globalTrace != null) {
			// 设置appName
			if( method.getDeclaringClass().getName().equals("org.apache.catalina.core.ApplicationFilterChain") && method.getName().equals("doFilter")) {
				if(globalTrace.getAppName() == null) {
					Object servletRequest = args[0];
					Map<String,Object> vars = new HashMap<String,Object>();
					vars.put("request", servletRequest);
					String contextPath = MVEL.eval("request.getContextPath()", vars) + "";
					if(contextPath.length() == 0) {
						contextPath = "/";
					}
					globalTrace.setAppName(contextPath.matches("\\/.+") ? contextPath.substring(1) : contextPath);
				}
			}
			// 设置httpComponent： spring mvc / struts /
			if(method.getDeclaringClass().getName().equals("org.springframework.web.servlet.DispatcherServlet") && method.getName().equals("doService")) {
				// spring mvc
				globalTrace.setHttpComponent("SpringMVC");
			} else if(method.getDeclaringClass().getName().matches("org.apache.struts2..*.StrutsActionProxy") && method.getName().equals("execute")) {
				// StrutsPrepareAndExecuteFilter
				globalTrace.setHttpComponent("Struts2");
				Object appName = MVEL.eval("org.apache.struts2.ServletActionContext.getServletContext().getServletContextName()") ;
				globalTrace.setAppName(appName == null || appName.equals("") ? "/" : appName.toString());
			}
			
			if(!method.getDeclaringClass().getName().matches(ignoreTraceNodePattern)) {
				recordTraceNode(traceNode,globalTrace,method);
			}
			
			// 下面分插件开发当前trace都在干什么事情
			/*mysql sql*/
			if(method.getDeclaringClass().getName().equals("com.mysql.jdbc.ConnectionImpl") && method.getName().equals("execSQL")) {
				if(args.length == 10) {
					recordTraceNode(traceNode,globalTrace,method);
					Object sql = args[1];
					...
					try {
						...
						if(statementImpl != null) {
							...
							if(sql == null) {
								sql = statementImpl.getClass().getMethod("asSql").invoke(statementImpl);
							}
							traceNode.setTraceCommand(sql+"");
							target = statementImpl.getClass().getMethod("getConnection").invoke(statementImpl);
							
							Map<String,Object> vars = new HashMap<String,Object>();
							vars.put("conn", target);
							Field passwordField = method.getDeclaringClass().getDeclaredField("password");
							passwordField.setAccessible(true);
							String password = passwordField.get(target) + "";
							
							Field portField = method.getDeclaringClass().getDeclaredField("port");
							...
							dbServer.setHost(MVEL.eval("conn.getHost()", vars) + "");
							dbServer.setPort(port);
							dbServer.setUrl(MVEL.eval("conn.getURL()", vars) + "");
							dbServer.setUser(MVEL.eval("conn.getUser()", vars) + "");
			...
			} else if(method.getDeclaringClass().getName().startsWith("redis.clients")) {
				// redis
				String methodName = method.getName();
				if(method.getDeclaringClass().getName().equals("redis.clients.jedis.BinaryJedis") && methodName.equals("connect")) {
					// 获取连接
//					recordTraceNode(traceNode,globalTrace,method);
//					traceNode.setModule("redis");
				} else if(method.getDeclaringClass().getName().equals("redis.clients.jedis.Jedis") /*&& method.getName().equals("sendCommand")*/) {
					if(args.length > 0) {
						recordTraceNode(traceNode,globalTrace,method);
						traceNode.setModule("redis");
						try {
							Field argument0 = callable.getClass().getDeclaredField("argument0");
							argument0.setAccessible(true);
							Object jedis = argument0.get(callable);
							
							Map<String,Object> vars = new HashMap<String,Object>();
							vars.put("jedis", jedis);
							String host = MVEL.eval("jedis.getClient().getHost()", vars) + "";
							String port = MVEL.eval("jedis.getClient().getPort()", vars) + "";
							
							traceNode.setTraceType("redis");
							traceNode.setTraceCommand(methodName + ":" + Arrays.asList(args));
		...
		/* httpclient */
		if(method.getDeclaringClass().getName().equals("org.apache.http.impl.client.CloseableHttpClient") && method.getName().equals("execute")) {
//			System.out.println(" sql args :" + new ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(args));
		    if(args.length == 2) {
		    	Map<String,Object> vars = new HashMap<String,Object>();
		    	vars.put("request", args[0]);
		    	System.out.println("httpclient url : " + MVEL.eval("request.getURI()", vars));
		    }
		}
		
		if(traceNode.getFullMethodName() != null) {
			StackTraceElement[] stackTraces = Thread.currentThread().getStackTrace();
//			System.out.println("======== current method : " + method.getDeclaringClass() + "." + method.getName());
...
//					boolean find = false;
//					for(int stackIndex = 3; stackIndex < stackTraces.length ; stackIndex++) {
//						StackTraceElement tempStackTrace = stackTraces[stackIndex];
//						if(tempStackTrace.getClassName().equals(temp.getClassName()) && tempStackTrace.getMethodName().equals(temp.getMethodName())) {
//							// if match break;
//							find = true;
//							break;
//						}
...
//		
		...
		try {
			...
			if(isTomcatRequestResource) {
				...
				int status = (Integer) MVEL.eval("response.getStatus()", vars);
			...
			if(isGlobalTrace) {
				...
				globalTrace.setTimeMillis(globalTrace.getEndTimeMillis() - globalTrace.getBeginTimeMillis());
				System.out.println(" globalTrace node size : " + globalTrace.getTraceNodes().size());
//			    System.out.println(new ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(globalTrace));
                // 异步推送数据
	...
	private static void recordTraceNode(TraceNode traceNode,
		...
		if(globalTrace.getTraceNodes().size() == 1) {
			globalTrace.setEntryStackTraceIndex(Thread.currentThread().getStackTrace().length - 4);
		}
